...
FROM (
    SELECT 
        ...
        TRIM(TO_CHAR(NVL(tc.monto_ticket, 0), '$99G999G999', 
            'NLS_NUMERIC_CHARACTERS = '',.''')) AS MONTO,

        TRIM(TO_CHAR(
            ROUND(CASE 
                WHEN tc.monto_ticket IS NULL OR tc.monto_ticket <= 50000 THEN 0
        ...
        TRIM(TO_CHAR(
            ROUND(t.sueldo_base * (1 + NVL(ba.porcentaje, 0))), 
            '$99G999G999', 'NLS_NUMERIC_CHARACTERS = '',.'''
...
ORDER BY 
    TO_NUMBER(REPLACE(REPLACE(MONTO, '$', ''), '.', '')) DESC,
    nombre_trabajador;

-- CASO 2: VISTAS

CREATE SYNONYM s_bono_escolar FOR bono_escolar;
CREATE SYNONYM s_asignacion_familiar FOR asignacion_familiar;


CREATE OR REPLACE VIEW v_aumentos_estudios AS
SELECT 
   TO_CHAR(t.numrut, '09G999G999', 'NLS_NUMERIC_CHARACTERS = '',.''') AS RUT_TRABAJADOR,
    ...
    TRIM(TO_CHAR(
        ...
        '$99G999G999', 'NLS_NUMERIC_CHARACTERS = '',.''')) AS SUELDO_AUMENTADO
...
-- ETAPA 2: Optimización de Consultas con Índices

CREATE INDEX idx_trabajador_apm ON trabajador(apmaterno);

CREATE INDEX idx_trabajador_apm_upper ON trabajador(UPPER(apmaterno));

EXEC DBMS_STATS.GATHER_INDEX_STATS(ownname => 'PRY2205_S7', indname => 'IDX_TRABAJADOR_APM');
EXEC DBMS_STATS.GATHER_INDEX_STATS(ownname => 'PRY2205_S7', indname => 'IDX_TRABAJADOR_APM_UPPER');

-- CONSULTAS DE VERIFICACIÓN Y PLANES DE EJECUCIÓN
